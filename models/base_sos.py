...
class NIKSoSBase(NIKBase):
    ...
    def load_names(self):
        ...
        exp_id = f"_hdr{self.config['hdr_ff_factor']}_slice{self.config['slice']}_" \
                 f"R{self.config['dataset']['acc_factor']}"
        ...
        # if "results_root" not in self.config:
        ...
        self.model_save_path = model_save_path if self.model_save_path is None else self.model_save_path
        self.result_save_path = results_save_path if self.result_save_path is None else self.result_save_path
    ...
    def save_network(self, name):
        ...
        torch.save({
            # 'epoch': epochs,
            'model_state_dict': self.network_kdata.state_dict(),
    ...
    def load_network(self):
        ...
        self.network_kdata.load_state_dict(torch.load(path, map_location=self.device)["model_state_dict"])
        if self.optimizer is not None:
            self.optimizer.load_state_dict(torch.load(path, map_location=self.device)["optimizer_state_dict"])
    ...
    def save_network(self, name, epoch, loss):
        """Save the network parameters to the path."""
        path = os.path.join(self.model_save_path, name)
        torch.save({
            'epoch': epoch,
