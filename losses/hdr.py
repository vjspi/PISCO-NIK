...
class HDRLoss_FF(torch.nn.Module):
    """
    HDR losses function with frequency filtering (v4)
    """
    ...
    def __init__(self, config):
        ...
        self.weigh_log = float(config['weigh_log']) if "weigh_log" in config else None
    ...
    def forward(self, input, target, kcoords, weights=None, reduce=True):
        ...
        if self.weigh_log is None:
            loss = (error.abs() / (input.detach().abs() + self.eps)) ** 2
        else:
            loss = (error.abs() / (self.weigh_log * input.detach().abs() + self.eps)) ** 2
